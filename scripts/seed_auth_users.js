...
(async ()=>{
  for(const u of users){
    ...
    await fetch(`${url}/rest/v1/profiles`, { method:'POST', headers:{ 'Content-Type':'application/json', 'apikey': adminKey, 'Authorization': 'Bearer ' + adminKey }, body: JSON.stringify({ auth_id: j.id, full_name: u.email.split('@')[0], email: u.email, actor_type: u.role, role_key: 'data_analyst', goal_clarity_score: 70 }) });
  }
